...
def righteousFurySpellAbilityBonus(attachee, args, evt_obj):
    evt_obj.bonus_list.add(4, 153, "~Righteous Fury~[TAG_SPELLS_RIGHTEOUS_FURY] ~Sacred~[TAG_MODIFIER_SACRED] Bonus") #Righteous Fury adds a +4 Sacred Bonus to Strength
    return 0

def righteousFurySpellOnConditionAdd(attachee, args, evt_obj):
    attachee.condition_add_with_args('Temporary_Hit_Points', 0, 600, args.get_arg(2)) #tempHp value is passed by spell (arg3), duration is up to 1 hour;
    return 0
