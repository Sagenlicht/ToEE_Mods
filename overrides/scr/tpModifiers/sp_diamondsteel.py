...
def diamondsteelSpellOnCondidtionAdd(attachee, args, evt_obj):
    ...
    print "Worn Armor: ", wornArmor
    acValue = wornArmor.obj_get_int(obj_f_armor_ac_adj)
    acValueQuery = attachee.d20_query(Q_Armor_Get_AC_Bonus)
    print "Armor Values: Armor {}, Query {}".format(acValue, acValueQuery)
    args.set_arg(2, acValue)
    return 0

def diamondsteelSpellOnBeginRound(attachee, args, evt_obj):
    print "Hook Begin Round"
    print "Value Arg3: ", args.get_arg(2)
...
diamondsteelSpell.AddHook(ET_OnBeginRound , EK_NONE, diamondsteelSpellOnBeginRound,())
